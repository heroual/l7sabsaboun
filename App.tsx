...
const Modal: React.FC<ModalProps> = ({ isOpen, onClose, title, children, zIndex = 50 }) => {
    if (!isOpen) return null;

    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center p-4" style={{ zIndex }} onClick={onClose}>
...
const ExpenseModal: React.FC<ExpenseModalProps> = ({ isOpen, onClose, onSave, expense, zIndex }) => {
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={expense && expense.id.startsWith('recurring-') ? 'إضافة فاتورة مستحقة' : expense ? 'تعديل المصروف' : 'مصروف جديد'} zIndex={zIndex}>
...
const IncomeModal: React.FC<IncomeModalProps> = ({ isOpen, onClose, onSave, income, zIndex }) => {
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={income && income.id.startsWith('recurring-') ? 'إضافة مدخول مستحق' : income ? 'تعديل المدخول' : 'مدخول جديد'} zIndex={zIndex}>
            <form onSubmit={handleSubmit} className="space-y-4">
                <div>
...
interface HistoryModalProps {
    ...
    onEditIncome: (income: IncomeSource) => void;
    onDeleteIncome: (incomeId: string) => void;
    onEditExpense: (expense: Expense) => void;
    onDeleteExpense: (expenseId: string) => void;
}

const HistoryModal: React.FC<HistoryModalProps> = ({ isOpen, onClose, record, onEditIncome, onDeleteIncome, onEditExpense, onDeleteExpense }) => {
                        ...
                        {record.incomeSources.length > 0 ? (
                            <ul className="space-y-1 text-sm">
                                {record.incomeSources.map(inc => (
                                    <li key={inc.id} className="flex justify-between items-center p-1.5 rounded bg-green-50 group">
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <button onClick={() => onEditIncome(inc)} className="text-slate-400 hover:text-blue-600">
                                                    <PencilIcon className="h-4 w-4" />
                                                </button>
                                                <button onClick={() => onDeleteIncome(inc.id)} className="text-slate-400 hover:text-red-600">
                        ...
                        {record.expenses.length > 0 ? (
                            <ul className="space-y-1 text-sm">
                                {record.expenses.map(exp => (
                                    <li key={exp.id} className="flex justify-between items-center p-1.5 rounded bg-red-50 group">
                                        <span>{String(exp.name)} ({exp.category})</span>
                                        <div className="flex items-center gap-2">
                                            <span className="font-semibold text-red-700">{Number(exp.amount).toFixed(2)}</span>
                                            <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <button onClick={() => onEditExpense(exp)} className="text-slate-400 hover:text-blue-600">
                                                    <PencilIcon className="h-4 w-4" />
                                                </button>
                                                <button onClick={() => onDeleteExpense(exp.id)} className="text-slate-400 hover:text-red-600">
                                                    <TrashIcon className="h-4 w-4" />
                                                </button>
                                            </div>
                                        </div>
...
const App: React.FC = () => {
    ...
    const [editingHistoricalMonthIndex, setEditingHistoricalMonthIndex] = useState<number | null>(null);
    ...
    const handleSaveExpense = (savedExpense: Omit<Expense, 'id' | 'date'> & { date: string }) => {
        // If editing historical data, use historical handler
        if (editingHistoricalMonthIndex !== null && editingExpense && !billToConvertToExpense) {
            handleSaveHistoricalExpense(savedExpense);
            return;
        }
    ...
    const handleSaveIncome = (savedIncome: Omit<IncomeSource, 'id' | 'date'> & { date: string }) => {
        // If editing historical data, use historical handler
        if (editingHistoricalMonthIndex !== null && editingIncome && !incomeToConvertToIncome) {
            handleSaveHistoricalIncome(savedIncome);
            return;
        }
    ...
    const handleOpenHistoryModal = (record: MonthlyRecord) => {
        const monthIndex = financeState.history.findIndex(r => r.month === record.month && r.year === record.year);
        setEditingHistoricalMonthIndex(monthIndex);
    ...
    const handleCloseHistoryModal = () => {
        ...
        setEditingHistoricalMonthIndex(null);
    }

    // Helper function to recalculate monthly record totals
    const recalculateMonthlyRecord = (record: MonthlyRecord): MonthlyRecord => {
        const totalIncome = record.incomeSources.reduce((sum, inc) => sum + inc.amount, 0);
    ...
    // Historical Income Handlers
    const handleEditHistoricalIncome = (income: IncomeSource) => {
        setEditingIncome(income);
        setIsIncomeModalOpen(true);
    };

    const handleDeleteHistoricalIncome = (incomeId: string) => {
        if (editingHistoricalMonthIndex === null) return;

        setFinanceState(prev => {
            const newHistory = [...prev.history];
            const record = newHistory[editingHistoricalMonthIndex];
            const updatedRecord = {
                ...record,
                incomeSources: record.incomeSources.filter(inc => inc.id !== incomeId)
            };
            newHistory[editingHistoricalMonthIndex] = recalculateMonthlyRecord(updatedRecord);
            setSelectedMonthRecord(newHistory[editingHistoricalMonthIndex]);
            return { ...prev, history: newHistory };
        });
    };

    const handleSaveHistoricalIncome = (savedIncome: Omit<IncomeSource, 'id' | 'date'> & { date: string }) => {
        if (editingHistoricalMonthIndex === null || !editingIncome) return;
        ...
        setFinanceState(prev => {
            ...
            const updatedRecord = {
                ...record,
                incomeSources: record.incomeSources.map(inc =>
                    inc.id === editingIncome.id ? { ...inc, ...savedIncome } : inc
                )
            };
            newHistory[editingHistoricalMonthIndex] = recalculateMonthlyRecord(updatedRecord);
            setSelectedMonthRecord(newHistory[editingHistoricalMonthIndex]);
    ...
    // Historical Expense Handlers
    const handleEditHistoricalExpense = (expense: Expense) => {
        setEditingExpense(expense);
        setIsExpenseModalOpen(true);
    };

    const handleDeleteHistoricalExpense = (expenseId: string) => {
        if (editingHistoricalMonthIndex === null) return;
        ...
        setFinanceState(prev => {
            ...
            const updatedRecord = {
                ...record,
                expenses: record.expenses.filter(exp => exp.id !== expenseId)
            };
            newHistory[editingHistoricalMonthIndex] = recalculateMonthlyRecord(updatedRecord);
            setSelectedMonthRecord(newHistory[editingHistoricalMonthIndex]);
            return { ...prev, history: newHistory };
    ...
    const handleSaveHistoricalExpense = (savedExpense: Omit<Expense, 'id' | 'date'> & { date: string }) => {
        if (editingHistoricalMonthIndex === null || !editingExpense) return;
        ...
        setFinanceState(prev => {
            ...
            const updatedRecord = {
                ...
                expenses: record.expenses.map(exp =>
                    exp.id === editingExpense.id ? { ...exp, ...savedExpense } : exp
                )
            };
            newHistory[editingHistoricalMonthIndex] = recalculateMonthlyRecord(updatedRecord);
            setSelectedMonthRecord(newHistory[editingHistoricalMonthIndex]);
    ...
    return (
        <div className="min-h-screen flex flex-col p-4 md:p-8 bg-slate-50 text-slate-800">
            ...
            <main className="flex-grow space-y-6">
                ...
                <AnalysisCard financeState={financeState} />
            </main>
            ...
            {isExpenseModalOpen && (
                <ExpenseModal
                    ...
                    zIndex={editingHistoricalMonthIndex !== null ? 60 : undefined}
                />
            )}
            {isIncomeModalOpen && (
                <IncomeModal
            ...
            {isHistoryModalOpen && (
                <HistoryModal
                    ...
                    onEditIncome={handleEditHistoricalIncome}
                    onDeleteIncome={handleDeleteHistoricalIncome}
                    onEditExpense={handleEditHistoricalExpense}
                    onDeleteExpense={handleDeleteHistoricalExpense}
                />
            )}
        </div>
    );
